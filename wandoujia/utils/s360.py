...
# 一级目录
cates = [
    {"cate_name": "软件", "url": "http://zhushou.360.cn/list/index/cid/1"},
    {"cate_name": "游戏", "url": "http://zhushou.360.cn/list/index/cid/2"}
]
...
def get_first_category():
    ...
    for cate in cates:
        ...
        # 解析HTML，获取二级分类
        categories = html.xpath('//ul[@class="select"]/li[1]/a')
        for category in categories:
            # 创建字典，存储三级目录信息
            dd = dict()
            if category.xpath('./text()') in [['全部']]:
                continue
            # 获取二级分类的名称
